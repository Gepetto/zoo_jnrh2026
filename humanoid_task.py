...
class Humanoid():
    ...
    def __init__(
        ...
        self.q_init = np.load("model/g1/reference_position.npy") # self.mj_model.qpos0.copy()
        state_size = mujoco.mj_stateSize(self.mj_model, mujoco.mjtState.mjSTATE_FULLPHYSICS)
        ...
        # control hold
        self.control_hold = np.zeros((N_ENVS, self.N_STEPS * self.N_HOLD, self.mj_model.nu))
    ...
    def set_goal_pos(self, q_goal):
        ...
        self.ref_left_foot_pos = np.array(self.mj_data.site_xpos[self.left_foot_site_id])
        self.ref_right_foot_pos = np.array(self.mj_data.site_xpos[self.right_foot_site_id])

    def _get_foot_position_errors(self, datas):        

        left_err  = np.array([np.linalg.norm(d.site_xpos[self.left_foot_site_id] - self.ref_left_foot_pos)**2 for d in datas])
        right_err = np.array([np.linalg.norm(d.site_xpos[self.right_foot_site_id] - self.ref_right_foot_pos)**2 for d in datas])
    ...
    def rollout(self, control):
        ...
        states, _ = rollout.rollout(self.mj_model, self.mj_datas, self.state_init, self.control_hold, nstep=self.N_STEPS*self.N_HOLD)
       
        cost = np.zeros(self.N_ENVS)

        for sim_t in range(self.N_STEPS*self.N_HOLD-1):
            qacc = (states[:, sim_t+1, self.mj_model.nq+1:] - states[:, sim_t, self.mj_model.nq+1:]) / self.dt
